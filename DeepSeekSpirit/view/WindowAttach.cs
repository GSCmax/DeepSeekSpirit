...
namespace DeepSeekSpirit
{
    ...
    public static class WindowAttach
    {
        ...
        private static void DragElement_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            if (sender is DependencyObject obj)
            {
                _pressedPosition = e.GetPosition(Window.GetWindow(obj));
            }
        }

        private static void DragElement_PreviewMouseMove(object sender, MouseEventArgs e)
        {
            if (sender is DependencyObject obj && e.LeftButton == MouseButtonState.Pressed && _pressedPosition != e.GetPosition(Window.GetWindow(obj)) && GetIsDragElement(sender as UIElement))
            {
                _isDragMoved = true;
                Window.GetWindow(obj).DragMove();
            }
        ...
        private static void OnShowInTaskManagerChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is Window window)
            {
                ...
                window.SetCurrentValue(Window.ShowInTaskbarProperty, v);
